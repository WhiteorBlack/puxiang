...
public class EmailViewModel extends BaseObservable implements ViewModel {
    ...
    public EmailViewModel(BaseBindActivity activity) {
    ...
    public void modifyName() {
        ...
        if (StringUtil.isEmpty(realName)) {
            ToastUtil.toast("邮箱地址不能为空");
            return;
        }
        if (!MyTextUtils.isEmail(realName)){
            ToastUtil.toast("请填写正确的邮箱地址");
            return;
        }
        ApiWrapper.getInstance()
                .modifyEmail(realName)
