...
import static com.puxiang.mall.utils.WebUtil.URL;
import static com.puxiang.mall.utils.WebUtil.verifyUrlSuffixed;

public class WebWithSearchViewModel extends BaseObservable implements ViewModel {
    ...
    public WebWithSearchViewModel(WebWithSearchActivity activity) {
        EventBus.getDefault().register(this);
        this.activity = activity;
        loadingWindow = new LoadingWindow(activity);
        initDialog();
        initData();
        isError.addOnPropertyChangedCallback(new OnPropertyChangedCallback() {
            @Override
    ...
    public WebViewClient getWebViewClient(ErrorShow errorShow) {
        return new WebViewClient() {
            ...
            public void onPageStarted(WebView view, String url, Bitmap favicon) {
                ...
                if (url.contains(URLs.HTML_SHOP_DETIAL)) {
                    toolBarTitle.set("店铺详情");
                    isShowSearch.set(false);
                }
                if (url.contains(URLs.HTML_GOODS_DETIAL)) {
                    toolBarTitle.set("商品详情");
                    isShowSearch.set(false);
                }
                if (url.contains(URLs.HTML_STOCK_LIST) || url.contains(URLs.HTML_SHOP_LIST)) {
                    isShowSearch.set(true);
                }
                if (url.contains(URLs.HTML_ORDER_DEALER_COMMIT)) {
                    toolBarTitle.set("确认订单");
                    isShowSearch.set(false);
                }

                if (url.contains(URLs.HTML_MY_GOOD_ADDRESS)) {
                    toolBarTitle.set("收货地址");
                    isShowSearch.set(false);
                }

                if (url.contains(URLs.HTML_MY_GOOD_ADDRESS_ADD)) {
                    toolBarTitle.set("新增地址");
                    isShowSearch.set(false);
                }
